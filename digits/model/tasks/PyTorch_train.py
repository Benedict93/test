...
# Constants
PYTORCH_MODEL_FILE = 'network.py'
PYTORCH_SNAPSHOT_PREFIX = 'snapshot'
...
@subclass
class PyTorchTrainTask(TrainTask):
    """
    Trains a PyTorch model
    """

    PYTORCH_LOG = 'PyTorch_output.log'
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(PyTorchTrainTask, self).__init__(**kwargs)

        # save network description to file
        with open(os.path.join(self.job_dir, PYTORCH_MODEL_FILE), "w") as outfile:
            outfile.write(self.network)

        self.pickver_task_pytorch_train = PICKLE_VERSION
    ...
    def __setstate__(self, state):
        super(PyTorchTrainTask, self).__setstate__(state)
    ...
    @override
    def name(self):
        return 'Train PyTorch Model'

    @override
    def before_run(self):
        super(PyTorchTrainTask, self).before_run()
        self.pytorch_log = open(self.path(self.PYTORCH_LOG), 'a')
    ...
    @staticmethod
    def preprocess_output_pytorch(line):
        """
        Takes line of output and parses it according to pytorch's output format
    ...
    def after_run(self):
        if self.temp_unrecognized_output:
            if self.traceback:
                ...
                self.traceback = '\n'.join(self.temp_unrecognized_output)
                self.temp_unrecognized_output = []
        self.pytorch_log.close()

    @override
    def after_runtime_error(self):
        if os.path.exists(self.path(self.PYTORCH_LOG)):
            output = subprocess.check_output(['tail', '-n40', self.path(self.PYTORCH_LOG)])
            lines = []
    ...
    def infer_one_image(self, image, snapshot_epoch=None, layers=None, gpu=None):
        ...
        """
        temp_image_handle, temp_image_path = tempfile.mkstemp(suffix='.png') # TO FIND OUT WHAT FORMAT for pytorch
    ...
    def infer_many_images(self, images, snapshot_epoch=None, gpu=None):
        ...
        temp_dir_path = tempfile.mkdtemp() # TO FIND OUT FOR PYTORCH
    ...
    def get_network_desc(self):
        ...
        """
        with open(os.path.join(self.job_dir, PYTORCH_MODEL_FILE), "r") as infile:
            desc = infile.read()
        return desc
